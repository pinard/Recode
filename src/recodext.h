...
typedef struct recode_charset *			RECODE_CHARSET;
...
enum recode_error
{
  ...
  RECODE_MAXIMUM_ERROR		/* impossible value (but should be kept last) */
};

/* Structure for relating charset or alias names to charsets.  */

struct recode_symbol
  {
    const char *name;		/* charset or alias name */
    RECODE_CHARSET charset;	/* associated charset */
    /* List of surfaces usually applied by default to the charset.  */
...
struct recode_outer
{
  ...
  /* Unique charsets are linked into a list and counted.  */
  RECODE_CHARSET charset_list;
...
struct recode_charset
{
  ...
  /* Type of table.  */
  enum recode_charset_type charset_type;
...
struct recode_request
{
  ...
  /* If producing a recoding table in source form, there will be no actual
     ...
     be attempted more aggressively.  If the step sequence cannot be
     reduced to a single step, table production will fail.  */
  ...
  /* For `ibmpc' charset, characters 176 to 223 are use to draw boxes.
     If this field is set, while getting out of `ibmpc', ASCII
...
#define IS_CHARSET(Outer, Charset) \
  ((Charset)->charset_flag)

#define IS_SURFACE(Outer, Charset) \
  ((Charset)->resurfacer || (Charset)->unsurfacer \
   || (Charset) == (Outer)->data_charset)

enum symbol_find_type
{
  SYMBOL_CREATE_CHARSET,	/* use exactly as given, create if necessary */
  SYMBOL_FIND_AS_CHARSET,	/* disambiguate only as a charset */
  SYMBOL_FIND_AS_SURFACE,	/* disambiguate only as a surface */
  SYMBOL_FIND_AS_EITHER		/* disambiguate as a charset or a surface */
};

int code_to_ucs2 (RECODE_CONST_CHARSET, unsigned);
bool prepare_for_names PARAMS ((RECODE_OUTER));
RECODE_SYMBOL declare_alias PARAMS ((RECODE_OUTER, const char *, const char *));
bool declare_implied_surface PARAMS ((RECODE_OUTER, RECODE_SYMBOL,
				    RECODE_CONST_CHARSET));
bool make_argmatch_arrays PARAMS ((RECODE_OUTER));
RECODE_SYMBOL find_symbol PARAMS ((RECODE_OUTER, const char *,
				   enum symbol_find_type));
